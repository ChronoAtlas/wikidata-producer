...
class RedisChecksumCache(ChecksumCache):
    ...
    def get_connection(self, force_reconnect=False) -> StrictRedis:
        if self.redis is not None and not force_reconnect:
            return self.redis

        while True:
            try:
                self.redis = StrictRedis.from_url(  # type: ignore (library devs don't know what typing is)
                    url=self.dsn,
                    encoding="utf-8",
                ...
                logging.error(error)
                logging.error(f"Unable to connect to redis. Retrying in 3 seconds")
                time.sleep(3)
